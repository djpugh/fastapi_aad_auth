...
class RoutingConfig(BaseSettings):
    ...
    token_path: str = Field('/token', description="Path for getting an access token for the user", env='FASTAPI_AUTH_TOKEN_ROUTE')
...
class LoginUIConfig(BaseSettings):
    ...
    token_template_file: FilePath = Field(resource_filename('fastapi_aad_auth.ui', 'token.html'),
                                          description="The jinja2 template to use",
                                          env='FASTAPI_AUTH_TOKEN_TEMPLATE_FILE')
